        ...
        <MidContentContainer>
          {userHistoricData.length !== 0 ? (
            userHistoricData.map((item) => {
              if (item.userId == -1) {
                //user.userId) {
                return <UserHistoricCard key={item._id} data={item} />;
              } else {
                return <h4> O seu histórico ainda está vazio. </h4>;
              }
            })
          ) : (
            <h4>O seu histórico ainda está vazio ou você não está logado.</h4>
          )}
          {/**
